 ...
 */
static __always_inline int my_get_netns_cookie(__maybe_unused const struct bpf_sock_addr *addr) {
...
#define SVC_KEY_VALUE(_port, _beslot, _beid, _scope) \
	{ \
		.key = {.address = v4_svc_one, .dport = (_port), .scope = (_scope), .backend_slot = (_beslot)}, .value = {.flags = SVC_FLAG_ROUTABLE, .count = 1, .rev_nat_index = 1, .backend_id = (_beid) } \
	}
