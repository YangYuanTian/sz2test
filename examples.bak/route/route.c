...
#include "bpf/include/bpf/ctx/xdp.h"
#include "bpf/include/bpf/helpers.h"
#include "bpf/lib/fib.h"
...
__section("xdp") int xdp_prog_func(struct xdp_md *ctx) {
	...
	__u32 *pkt_count = map_lookup_elem(&xdp_stats_map, &ip);
	if (!pkt_count) {
		// No entry in the map for this IP address yet, so set the initial value to 1.
		__u32 init_pkt_count = 1;
		map_update_elem(&xdp_stats_map, &ip, &init_pkt_count, BPF_ANY);
	} else {
