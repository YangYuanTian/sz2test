...
int test1_setup(struct __ctx_buff *ctx) {
	...
	struct ethhdr l2 = {.h_source = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}, .h_dest = {0x12, 0x23, 0x34, 0x45, 0x56, 0x67}, .h_proto = bpf_htons(ETH_P_IP)};
	...
	struct lb4_key lb_svc_key = {.address = 0x0F00010A, .dport = 80, .scope = LB_LOOKUP_SCOPE_EXT};
