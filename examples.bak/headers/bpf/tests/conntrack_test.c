...
int bpf_test(__maybe_unused struct __sk_buff *sctx) {
	...
	TEST("ct_lookup", {
		...
		int res;
		struct ipv4_ct_tuple tuple = {.nexthdr = IPPROTO_TCP};
		...
		res = __ct_lookup(get_ct_map4(&tuple), &ctx, &tuple, ACTION_CREATE, CT_INGRESS, &ct_state, true, seen_flags, &monitor);
		...
		res = __ct_lookup(get_ct_map4(&tuple), &ctx, &tuple, ACTION_CLOSE, CT_INGRESS, &ct_state, true, seen_flags, &monitor);
		assert(res == CT_ESTABLISHED);
		assert(monitor == TRACE_PAYLOAD_LEN);
		assert(timeout_in(entry, CT_CONNECTION_LIFETIME_TCP));
		...
		advance_time();
		seen_flags.value |= TCP_FLAG_FIN;
		res = __ct_lookup(get_ct_map4(&tuple), &ctx, &tuple, ACTION_CLOSE, CT_EGRESS, &ct_state, true, seen_flags, &monitor);
		assert(res == CT_ESTABLISHED);
		assert(monitor == TRACE_PAYLOAD_LEN);
		...
		monitor = 0;
		seen_flags.value &= ~TCP_FLAG_FIN;
		res = __ct_lookup(get_ct_map4(&tuple), &ctx, &tuple, ACTION_CREATE, CT_EGRESS, &ct_state, true, seen_flags, &monitor);
		assert(res == CT_ESTABLISHED);
		assert(monitor == 0);
