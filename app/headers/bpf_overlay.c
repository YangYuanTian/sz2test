...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV6_FROM_OVERLAY) int tail_handle_ipv6(struct __ctx_buff
                                                            *ctx) {
  __u32 src_identity = 0;
  int ret = handle_ipv6(ctx, &src_identity);

  if (IS_ERR(ret))
    return send_drop_notify_error(ctx, src_identity, ret, CTX_ACT_DROP,
                                  METRIC_INGRESS);
  return ret;
}
#endif /* ENABLE_IPV6 */
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV4_FROM_OVERLAY) int tail_handle_ipv4(struct __ctx_buff
                                                            *ctx) {
  __u32 src_identity = 0;
...
__section_tail(CILIUM_MAP_CALLS,
  ...
  return send_drop_notify_error(ctx, 0, DROP_UNKNOWN_L3, CTX_ACT_DROP,
                                METRIC_EGRESS);
 ...
 */
__section("from-overlay") int cil_from_overlay(struct __ctx_buff *ctx) {
  __u16 proto;
  int ret;
...
#ifdef ENABLE_IPSEC
  if (is_esp(ctx, proto))
    send_trace_notify(ctx, TRACE_FROM_OVERLAY, 0, 0, 0, ctx->ingress_ifindex,
                      TRACE_REASON_ENCRYPTED, TRACE_PAYLOAD_LEN);
  else
#endif
  {
    ...
    send_trace_notify(ctx, obs_point, identity, 0, 0, ctx->ingress_ifindex,
 ...
 */
__section("to-overlay") int cil_to_overlay(struct __ctx_buff *ctx) {
  int ret;
