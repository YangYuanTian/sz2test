...
int test1_setup(struct __ctx_buff *ctx) {
  ...
  struct ethhdr l2 = {.h_source = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF},
  ...
  struct lb4_key lb_svc_key = {
      .address = 0x0F00010A, .dport = 80, .scope = LB_LOOKUP_SCOPE_EXT};
  ...
  map_update_elem(&LB4_BACKEND_MAP_V2, &lb_svc_value.backend_id, &backend,
                  BPF_ANY);
