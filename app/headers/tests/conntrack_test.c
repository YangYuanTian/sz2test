...
void advance_time(void) { __now = __now + 1; }
...
int bpf_test(__maybe_unused struct __sk_buff *sctx) {
  ...
  TEST("ct_update_timeout", {
     ...
     */
    monitor =
        __ct_update_timeout(&entry, 1000, CT_INGRESS, flags, REPORT_ALL_FLAGS);
    assert(!monitor);
    ...
    monitor =
        __ct_update_timeout(&entry, 1000, CT_INGRESS, flags, REPORT_NO_FLAGS);
    assert(!monitor);
    assert(entry.rx_flags_seen == bpf_ntohs(TCP_FLAG_SYN));
    assert(entry.tx_flags_seen == 0);
  });
  ...
  TEST("ct_lookup", {
    ...
    res = __ct_lookup(get_ct_map4(&tuple), &ctx, &tuple, ACTION_CREATE,
                      CT_INGRESS, &ct_state, true, seen_flags, &monitor);
    ...
    res = __ct_lookup(get_ct_map4(&tuple), &ctx, &tuple, ACTION_CLOSE,
                      CT_INGRESS, &ct_state, true, seen_flags, &monitor);
    assert(res == CT_ESTABLISHED);
    assert(monitor == TRACE_PAYLOAD_LEN);
    assert(timeout_in(entry, CT_CONNECTION_LIFETIME_TCP));
    ...
    seen_flags.value |= TCP_FLAG_FIN;
    res = __ct_lookup(get_ct_map4(&tuple), &ctx, &tuple, ACTION_CLOSE,
                      CT_EGRESS, &ct_state, true, seen_flags, &monitor);
    assert(res == CT_ESTABLISHED);
    assert(monitor == TRACE_PAYLOAD_LEN);
