 ...
 * It contains function definitions for testing "send_drop_notify" and
 * "__send_drop_notify". It is used to perform unit test on the above two
 * functions to demonstrate how to handle tailcalls. There is a tailcall at the
 * end of function send_drop_notif which actually calls function
 * __send_drop_notify. We can stub the tailcall and actually call the function
 * with callback. If other functions in drop.h need to be tested, please add the
...
/* Define macros like the following to make sure the original tailcall is
 * redirected */
...
void mock_tail_call(void *ctx, __maybe_unused const void *map,
                    __maybe_unused __u32 index) {
