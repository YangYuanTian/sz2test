...
static __always_inline bool allow_vlan(__u32 __maybe_unused ifindex,
...
#ifdef ENABLE_IPV6
static __always_inline __u32 derive_src_id(const union v6addr *node_ip,
                                           struct ipv6hdr *ip6,
                                           __u32 *identity) {
...
static __always_inline __u32 resolve_srcid_ipv6(struct __ctx_buff *ctx,
  ...
  else if (src_id == WORLD_ID && identity_from_ipcache_ok() &&
           !identity_is_reserved(srcid_from_ipcache))
    src_id = srcid_from_ipcache;
  return src_id;
}

static __always_inline int handle_ipv6(struct __ctx_buff *ctx, __u32 secctx,
                                       const bool from_host) {
...
static __always_inline int tail_handle_ipv6(struct __ctx_buff *ctx,
                                            const bool from_host) {
  ...
  if (IS_ERR(ret))
    return send_drop_notify_error(ctx, proxy_identity, ret, CTX_ACT_DROP,
                                  METRIC_INGRESS);
  return ret;
}
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV6_FROM_HOST) int tail_handle_ipv6_from_host(struct
                                                               __ctx_buff *ctx
                                                                   __maybe_unused) {
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV6_FROM_NETDEV) int tail_handle_ipv6_from_netdev(struct
                                                                   __ctx_buff
                                                                       *ctx) {
...
#ifdef ENABLE_HOST_FIREWALL
static __always_inline int handle_to_netdev_ipv6(struct __ctx_buff *ctx,
...
#ifdef ENABLE_IPV4
static __always_inline __u32 resolve_srcid_ipv4(struct __ctx_buff *ctx,
                                                __u32 srcid_from_proxy,
                                                __u32 *sec_label,
                                                const bool from_host) {
  __u32 src_id = WORLD_ID, srcid_from_ipcache = srcid_from_proxy;
...
static __always_inline int handle_ipv4(struct __ctx_buff *ctx, __u32 secctx,
  ...
  if (!from_host) {
    if (ctx_get_xfer(ctx) != XFER_PKT_NO_SVC && !bpf_skip_nodeport(ctx)) {
      ...
      if (ret == NAT_46X64_RECIRC) {
        ...
        return send_drop_notify_error(ctx, secctx, DROP_MISSED_TAIL_CALL,
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV4_FROM_HOST) int tail_handle_ipv4_from_host(struct __ctx_buff
                                                                   *ctx) {
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV4_FROM_NETDEV) int tail_handle_ipv4_from_netdev(struct
                                                                   __ctx_buff
                                                                       *ctx) {
...
#ifdef ENABLE_HOST_FIREWALL
static __always_inline int handle_to_netdev_ipv4(struct __ctx_buff *ctx,
                                                 struct trace_ctx *trace) {
  void *data, *data_end;
  struct iphdr *ip4;
...
do_netdev_encrypt_pools(struct __ctx_buff *ctx __maybe_unused) {
  ...
  if (l3_csum_replace(ctx, ETH_HLEN + offsetof(struct iphdr, check), 0, sum,
                      0) < 0) {
    ret = DROP_CSUM_L3;
    goto drop_err;
  }
...
static __always_inline int do_netdev_encrypt_fib(
    struct __ctx_buff *ctx __maybe_unused, __u16 proto __maybe_unused,
    int *encrypt_iface __maybe_unused, int *ext_err __maybe_unused) {
  int ret = 0;
...
static __always_inline int do_netdev(struct __ctx_buff *ctx, __u16 proto,
  ...
  if (from_host) {
    ...
    send_trace_notify(ctx, TRACE_FROM_NETWORK, 0, 0, 0, ctx->ingress_ifindex,
                      TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN);
  }
  ...
  default:
#ifdef ENABLE_HOST_FIREWALL
    ret = send_drop_notify_error(ctx, identity, DROP_UNKNOWN_L3, CTX_ACT_DROP,
                                 METRIC_INGRESS);
#else
 ...
 */
static __always_inline int handle_netdev(struct __ctx_buff *ctx,
                                         const bool from_host) {
  __u16 proto;
  ...
  if (!validate_ethertype(ctx, &proto)) {
    ...
    return send_drop_notify(ctx, SECLABEL, WORLD_ID, 0, ret, CTX_ACT_DROP,
                            METRIC_EGRESS);
#else
...
#ifdef ENABLE_SRV6
static __always_inline int handle_srv6(struct __ctx_buff *ctx) {
  __u32 *vrf_id, dst_id;
...
/*
 * from-netdev is attached as a tc ingress filter to one or more physical
 * devices managed by Cilium (e.g., eth0). This program is only attached when:
 * - the host firewall is enabled, or
 * - BPF NodePort is enabled
 */
__section("from-netdev") int cil_from_netdev(struct __ctx_buff *ctx) {
  __u32 __maybe_unused vlan_id;
  ...
  if (ctx->vlan_present) {
    ...
    if (vlan_id) {
      if (allow_vlan(ctx->ifindex, vlan_id))
        ...
        return send_drop_notify_error(ctx, 0, DROP_VLAN_FILTERED, CTX_ACT_DROP,
                                      METRIC_INGRESS);
    }
  }
 ...
 */
__section("from-host") int cil_from_host(struct __ctx_buff *ctx) {
 ...
 * - BPF NodePort is enabled
 */
__section("to-netdev") int cil_to_netdev(
...
out:
  ...
  if (ret != CTX_ACT_OK)
    ...
    (!defined(ENABLE_DSR) ||                                                   \
     ...
     defined(ENABLE_MASQUERADE) || defined(ENABLE_EGRESS_GATEWAY))
 ...
 */
__section("to-host") int cil_to_host(struct __ctx_buff *ctx) {
...
out:
  ...
  if (!traced)
    send_trace_notify(ctx, TRACE_TO_STACK, src_id, 0, 0, CILIUM_IFINDEX,
...
declare_tailcall_if(
    __or(__and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)),
         is_defined(DEBUG)),
    CILIUM_CALL_IPV6_TO_HOST_POLICY_ONLY) int tail_ipv6_host_policy_ingress(struct
                                                                            __ctx_buff
                                                                                *ctx) {
  struct trace_ctx __maybe_unused trace = {
      .reason = TRACE_REASON_UNKNOWN,
...
declare_tailcall_if(
    __or(__and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)),
         is_defined(DEBUG)),
    CILIUM_CALL_IPV4_TO_HOST_POLICY_ONLY) int tail_ipv4_host_policy_ingress(struct
                                                                            __ctx_buff
                                                                                *ctx) {
  struct trace_ctx __maybe_unused trace = {
      .reason = TRACE_REASON_UNKNOWN,
...
to_host_from_lxc(struct __ctx_buff *ctx __maybe_unused)
{
  ...
  case bpf_htons(ETH_P_IPV6):
    invoke_tailcall_if(
        __or(__and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)),
             is_defined(DEBUG)),
        CILIUM_CALL_IPV6_TO_HOST_POLICY_ONLY, tail_ipv6_host_policy_ingress);
    break;
#endif
#ifdef ENABLE_IPV4
  case bpf_htons(ETH_P_IP):
    invoke_tailcall_if(
        __or(__and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)),
             is_defined(DEBUG)),
        CILIUM_CALL_IPV4_TO_HOST_POLICY_ONLY, tail_ipv4_host_policy_ingress);
    break;
#endif
  default:
    ret = DROP_UNKNOWN_L3;
 ...
 * bpf_lxc program.
 */
__section_tail(CILIUM_MAP_POLICY, TEMPLATE_HOST_EP_ID) int handle_lxc_traffic(
