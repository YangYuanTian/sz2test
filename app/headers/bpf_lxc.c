...
/* Override LB_SELECTION initially defined in node_config.h to force bpf_lxc to
 * use the random backend selection algorithm for in-cluster traffic. Otherwise,
 * it will fail with the Maglev hash algorithm because Cilium doesn't provision
...
#define TAIL_CT_LOOKUP4(ID, NAME, DIR, CONDITION, TARGET_ID, TARGET_NAME)      \
  declare_tailcall_if(CONDITION, ID) int NAME(struct __ctx_buff *ctx) {        \
    ...
    l4_off = ETH_HLEN + ipv4_hdrlen(ip4);                                      \
                                                                               \
    ct_buffer.ret = ct_lookup4(get_ct_map4(tuple), tuple, ctx, l4_off, DIR,    \
...
#define TAIL_CT_LOOKUP6(ID, NAME, DIR, CONDITION, TARGET_ID, TARGET_NAME)      \
  declare_tailcall_if(CONDITION, ID) int NAME(struct __ctx_buff *ctx) {        \
    ...
    l4_off = ETH_HLEN + hdrlen;                                                \
                                                                               \
    ct_buffer.ret = ct_lookup6(get_ct_map6(tuple), tuple, ctx, l4_off, DIR,    \
                               ct_state, &ct_buffer.monitor);                  \
    if (ct_buffer.ret < 0)                                                     \
      return ct_buffer.ret;                                                    \
                                                                               \
...
static __always_inline bool redirect_to_proxy(int verdict,
  ...
  return verdict > 0 && (status == CT_NEW || status == CT_ESTABLISHED ||
                         status == CT_REOPENED);
}
#endif
 ...
 * convenience.
 */
static __always_inline int encode_custom_prog_meta(struct __ctx_buff *ctx,
                                                   int ret, __u32 identity) {
  __u32 custom_meta = 0;
...
/* Handle egress IPv6 traffic from a container after service translation has
 * been done either at the socket level or by the caller. In the case of the
 * caller doing the service translation it passes in state via CB, which we take
 * in with lb6_ctx_restore_state().
 *
 * Kernel 4.9 verifier is very finicky about the order of this code, modify with
 * caution.
 */
static __always_inline int handle_ipv6_from_lxc(struct __ctx_buff *ctx,
                                                __u32 *dst_id) {
  struct ct_state ct_state_on_stack __maybe_unused, *ct_state,
      ct_state_new = {};
  ...
  bool hairpin_flow =
      false; /* endpoint wants to access itself via service IP */
  ...
  if (ct_status != CT_REPLY && ct_status != CT_RELATED && verdict < 0) {
    send_policy_verdict_notify(ctx, *dst_id, tuple->dport, tuple->nexthdr,
...
skip_policy_enforcement:
  ...
  ct_recreate6:
    ...
    ret = ct_create6(get_ct_map6(tuple), &CT_MAP_ANY6, tuple, ctx, CT_EGRESS,
                     &ct_state_new, verdict > 0, from_l7lb);
    if (IS_ERR(ret))
      return ret;
  ...
  case CT_REPLY:
    ...
    if (ct_state->dsr) {
      ...
      if (ct_state->node_port) {
        send_trace_notify(ctx, TRACE_TO_NETWORK, SECLABEL, *dst_id, 0, 0,
    ...
    if (ct_state->rev_nat_index) {
      ...
      ret = lb6_rev_nat(ctx, l4_off, &csum_off, ct_state->rev_nat_index, tuple,
                        0);
      if (IS_ERR(ret))
        return ret;
  ...
  if (!from_l7lb && redirect_to_proxy(verdict, ct_status)) {
    ...
    send_trace_notify(ctx, TRACE_TO_PROXY, SECLABEL, 0, bpf_ntohs(proxy_port),
  ...
  if (is_defined(ENABLE_ROUTING) || hairpin_flow) {
    ...
    if (ep) {
      ...
      return ipv6_local_delivery(ctx, ETH_HLEN, SECLABEL, ep, METRIC_EGRESS,
                                 from_l7lb);
    }
  }
...
#endif /* ENABLE_WIREGUARD */
  {
     ...
     */
    ret = encap_and_redirect_lxc(ctx, tunnel_endpoint, encrypt_key, &key,
                                 SECLABEL, &trace);
  ...
  if (is_defined(ENABLE_HOST_ROUTING)) {
    ...
    if (likely(ret == CTX_ACT_REDIRECT))
      send_trace_notify(ctx, TRACE_TO_NETWORK, SECLABEL, *dst_id, 0, oif,
                        trace.reason, trace.monitor);
    return ret;
  }
...
encrypt_to_stack:
#endif
  send_trace_notify(ctx, TRACE_TO_STACK, SECLABEL, *dst_id, 0, 0, trace.reason,
                    trace.monitor);
...
declare_tailcall_if(
    ...
    CILIUM_CALL_IPV6_FROM_LXC_CONT) int tail_handle_ipv6_cont(struct __ctx_buff
                                                                  *ctx) {
  __u32 dst_id = 0;
  int ret = handle_ipv6_from_lxc(ctx, &dst_id);

  if (IS_ERR(ret))
    return send_drop_notify(ctx, SECLABEL, dst_id, 0, ret, CTX_ACT_DROP,
                            METRIC_EGRESS);
...
#endif /* ENABLE_L7_LB */
      ret = lb6_local(get_ct_map6(&tuple), ctx, ETH_HLEN, l4_off, &csum_off,
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV6_FROM_LXC) int tail_handle_ipv6(struct __ctx_buff *ctx) {
...
skip_policy_enforcement:
  ...
  case CT_REPLY:
    ...
    if (ct_state->rev_nat_index) {
      ...
      csum_l4_offset_and_flags(tuple->nexthdr, &csum_off);
      ret = lb4_rev_nat(ctx, ETH_HLEN, l4_off, &csum_off, ct_state, tuple, 0,
                        has_l4_header);
      if (IS_ERR(ret))
        return ret;
    }
...
declare_tailcall_if(
    is_defined(ENABLE_PER_PACKET_LB),
    CILIUM_CALL_IPV4_FROM_LXC_CONT) int tail_handle_ipv4_cont(struct __ctx_buff
                                                                  *ctx) {
  __u32 dst_id = 0;
...
#endif /* ENABLE_L7_LB */
      ret = lb4_local(get_ct_map4(&tuple), ctx, ETH_HLEN, l4_off, &csum_off,
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV4_FROM_LXC) int tail_handle_ipv4(struct __ctx_buff *ctx) {
 ...
 */
__section_tail(CILIUM_MAP_CALLS,
               CILIUM_CALL_ARP) int tail_handle_arp(struct __ctx_buff *ctx) {
 ...
 */
__section("from-container") int cil_from_container(struct __ctx_buff *ctx) {
  __u16 proto;
  int ret;
...
ipv6_policy(struct __ctx_buff *ctx, int ifindex, __u32 src_label,
  ...
  *ct_status = (enum ct_status)ret;
  ...
  if ((ret == CT_REPLY || ret == CT_RELATED) &&
     ...
     */
    send_trace_notify6(ctx, TRACE_TO_PROXY, src_label, SECLABEL, &orig_sip, 0,
                       ifindex, (enum trace_reason)ret, monitor);
    if (tuple_out)
  ...
  if (unlikely(ct_state->rev_nat_index)) {
    ...
    ret2 =
        lb6_rev_nat(ctx, l4_off, &csum_off, ct_state->rev_nat_index, tuple, 0);
    if (IS_ERR(ret2))
      return ret2;
  }

  verdict = policy_can_access_ingress(ctx, src_label, SECLABEL, tuple->dport,
  ...
  send_trace_notify6(ctx, TRACE_TO_LXC, src_label, SECLABEL, &orig_sip, LXC_ID,
                     ifindex, reason, monitor);

#if !defined(ENABLE_ROUTING) && defined(TUNNEL_MODE) &&                        \
    !defined(ENABLE_NODEPORT)
...
declare_tailcall_if(
    ...
    CILIUM_CALL_IPV6_TO_LXC_POLICY_ONLY) int tail_ipv6_policy(struct __ctx_buff
                                                                  *ctx) {
  struct ipv6_ct_tuple tuple = {};
  ...
  ret = ipv6_policy(ctx, ifindex, src_label, &ct_status, &tuple, &proxy_port,
                    from_host);
  ...
  if (IS_ERR(ret))
    return send_drop_notify(ctx, src_label, SECLABEL, LXC_ID, ret, CTX_ACT_DROP,
                            METRIC_INGRESS);
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV6_TO_ENDPOINT) int tail_ipv6_to_endpoint(struct __ctx_buff
                                                                *ctx) {
...
out:
  if (IS_ERR(ret))
    return send_drop_notify(ctx, src_identity, SECLABEL, LXC_ID, ret,
...
ipv4_policy(struct __ctx_buff *ctx, int ifindex, __u32 src_label,
            enum ct_status *ct_status, struct ipv4_ct_tuple *tuple_out,
            __u16 *proxy_port, bool from_host __maybe_unused) {
  struct ct_state ct_state_on_stack __maybe_unused, *ct_state,
      ct_state_new = {};
  struct ipv4_ct_tuple tuple_on_stack __maybe_unused, *tuple;
  ...
  *ct_status = (enum ct_status)ret;
  ...
  if ((ret == CT_REPLY || ret == CT_RELATED) &&
     ...
     * into ctx->mark and *proxy_port can be left unset.
     */
    send_trace_notify4(ctx, TRACE_TO_PROXY, src_label, SECLABEL, orig_sip, 0,
                       ifindex, (enum trace_reason)ret, monitor);
    if (tuple_out)
      *tuple_out = *tuple;
    return POLICY_ACT_PROXY_REDIRECT;
  ...
  if (unlikely(ret == CT_REPLY && ct_state->rev_nat_index &&
    ...
    ret2 = lb4_rev_nat(ctx, ETH_HLEN, l4_off, &csum_off, ct_state, tuple,
  ...
  verdict = policy_can_access_ingress(ctx, src_label, SECLABEL, tuple->dport,
                                      tuple->nexthdr, is_untracked_fragment,
                                      &policy_match_type, &audited);
...
skip_policy_enforcement:
  ...
  /* Not redirected to host / proxy. */
  send_trace_notify4(ctx, TRACE_TO_LXC, src_label, SECLABEL, orig_sip, LXC_ID,
                     ifindex, reason, monitor);
...
declare_tailcall_if(
    __and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)),
    CILIUM_CALL_IPV4_TO_LXC_POLICY_ONLY) int tail_ipv4_policy(struct __ctx_buff
                                                                  *ctx) {
  struct ipv4_ct_tuple tuple = {};
  int ret, ifindex = ctx_load_meta(ctx, CB_IFINDEX);
  __u32 src_label = ctx_load_meta(ctx, CB_SRC_LABEL);
  ...
  ret = ipv4_policy(ctx, ifindex, src_label, &ct_status, &tuple, &proxy_port,
                    from_host);
  if (ret == POLICY_ACT_PROXY_REDIRECT) {
...
__section_tail(
    CILIUM_MAP_CALLS,
    CILIUM_CALL_IPV4_TO_ENDPOINT) int tail_ipv4_to_endpoint(struct __ctx_buff
                                                                *ctx) {
  __u32 src_identity = ctx_load_meta(ctx, CB_SRC_LABEL);
  bool proxy_redirect __maybe_unused = false;
  void *data, *data_end;
 ...
 */
__section_tail(CILIUM_MAP_POLICY,
               TEMPLATE_LXC_ID) int handle_policy(struct __ctx_buff *ctx) {
  __u32 src_label = ctx_load_meta(ctx, CB_SRC_LABEL);
  __u16 proto;
  int ret;
...
#if defined(ENABLE_L7_LB)
__section_tail(
    CILIUM_MAP_EGRESSPOLICY,
    TEMPLATE_LXC_ID) int handle_policy_egress(struct __ctx_buff *ctx) {
  __u16 proto;
  int ret;
  ...
  send_trace_notify(ctx, TRACE_FROM_PROXY, SECLABEL, 0, 0, 0 /*ifindex*/,
                    TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN);
...
out:
  if (IS_ERR(ret))
    return send_drop_notify(ctx, SECLABEL, 0, LXC_ID, ret, CTX_ACT_DROP,
                            METRIC_EGRESS);
 ...
 * routes are enabled.
 */
__section("to-container") int cil_to_container(struct __ctx_buff *ctx) {
...
out:
  if (IS_ERR(ret))
    return send_drop_notify(ctx, identity, SECLABEL, LXC_ID, ret, CTX_ACT_DROP,
                            METRIC_INGRESS);
