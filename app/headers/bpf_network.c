...
__section("from-network") int cil_from_network(struct __ctx_buff *ctx) {
  int ret = CTX_ACT_OK;
...
out:
  send_trace_notify(ctx, obs_point_from, 0, 0, 0, ctx->ingress_ifindex, reason,
                    TRACE_PAYLOAD_LEN);

  send_trace_notify(ctx, obs_point_to, 0, 0, 0, ctx->ingress_ifindex, reason,
                    TRACE_PAYLOAD_LEN);
