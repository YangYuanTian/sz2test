 ...
 */
#if !defined(ENABLE_SOCKET_LB_FULL) || defined(ENABLE_SOCKET_LB_HOST_ONLY) || defined(ENABLE_L7_LB)
 ...
 */
#define HAVE_DIRECT_ACCESS_TO_MAP_VALUES HAVE_PROG_TYPE_HELPER(sched_cls, bpf_fib_lookup)
...
\
		ct_buffer.ret = ct_lookup4(get_ct_map4(tuple), tuple, ctx, l4_off, DIR, ct_state, &ct_buffer.monitor); \
		if (ct_buffer.ret < 0) \
...
\
		ct_buffer.ret = ct_lookup6(get_ct_map6(tuple), tuple, ctx, l4_off, DIR, ct_state, &ct_buffer.monitor); \
		if (ct_buffer.ret < 0) \
			return ct_buffer.ret; \
\
...
static __always_inline bool redirect_to_proxy(int verdict, enum ct_status status) {
	return verdict > 0 && (status == CT_NEW || status == CT_ESTABLISHED || status == CT_REOPENED);
}
#endif
...
declare_tailcall_if(is_defined(ENABLE_PER_PACKET_LB), CILIUM_CALL_IPV6_FROM_LXC_CONT) int tail_handle_ipv6_cont(struct __ctx_buff *ctx) {
	__u32 dst_id = 0;
...
declare_tailcall_if(is_defined(ENABLE_PER_PACKET_LB), CILIUM_CALL_IPV4_FROM_LXC_CONT) int tail_handle_ipv4_cont(struct __ctx_buff *ctx) {
	__u32 dst_id = 0;
...
declare_tailcall_if(__and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)), CILIUM_CALL_IPV6_TO_LXC_POLICY_ONLY) int tail_ipv6_policy(struct __ctx_buff *ctx) {
	struct ipv6_ct_tuple tuple = {};
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV6_TO_ENDPOINT) int tail_ipv6_to_endpoint(struct __ctx_buff *ctx) {
...
declare_tailcall_if(__and(is_defined(ENABLE_IPV4), is_defined(ENABLE_IPV6)), CILIUM_CALL_IPV4_TO_LXC_POLICY_ONLY) int tail_ipv4_policy(struct __ctx_buff *ctx) {
	struct ipv4_ct_tuple tuple = {};
	int ret, ifindex = ctx_load_meta(ctx, CB_IFINDEX);
	__u32 src_label                    = ctx_load_meta(ctx, CB_SRC_LABEL);
	bool from_host                     = ctx_load_meta(ctx, CB_FROM_HOST);
...
__section_tail(CILIUM_MAP_CALLS, CILIUM_CALL_IPV4_TO_ENDPOINT) int tail_ipv4_to_endpoint(struct __ctx_buff *ctx) {
	__u32 src_identity                 = ctx_load_meta(ctx, CB_SRC_LABEL);
	bool proxy_redirect __maybe_unused = false;
	void *data, *data_end;
	struct iphdr *ip4;
 ...
 */
#if defined(ENABLE_L7_LB)
__section_tail(CILIUM_MAP_EGRESSPOLICY, TEMPLATE_LXC_ID) int handle_policy_egress(struct __ctx_buff *ctx) {
	__u16 proto;
	int ret;
	...
	send_trace_notify(ctx, TRACE_FROM_PROXY, SECLABEL, 0, 0, 0 /*ifindex*/, TRACE_REASON_UNKNOWN, TRACE_PAYLOAD_LEN);
