...
long mock_fib_lookup(__maybe_unused void *ctx, struct bpf_fib_lookup *params, __maybe_unused int plen, __maybe_unused __u32 flags) {
...
static __always_inline int craft_packet(struct __ctx_buff *ctx) {
	...
	if (!eh)
		return TEST_ERROR;
	*eh = (struct ethhdr){.h_source = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}, .h_dest = {0x12, 0x23, 0x34, 0x45, 0x56, 0x67}, .h_proto = bpf_htons(ETH_P_IP)};
...
#define SVC_KEY_VALUE(_beslot, _beid, _scope) \
	{ \
		.key = {.address = FRONTEND_IP, .dport = FRONTEND_PORT, .scope = (_scope), .backend_slot = (_beslot)}, .value = {.flags = SVC_FLAG_ROUTABLE | SVC_FLAG_AFFINITY, .count = 2, .rev_nat_index = REV_NAT_INDEX, .backend_id = (_beid) } \
	}

#define BE_KEY_VALUE(_beid, _beip) \
	{ .key = (_beid), .value = {.address = (_beip), .port = BACKEND_PORT, .proto = IPPROTO_TCP}, }
